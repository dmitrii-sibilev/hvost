...
namespace Starlabs\Project\Crest;
...
class CRest
{
    ...
    public function processBatchCalls($halt = 0, $delay = 100000)
    {

        $batchQueryCounter = 0;
        while (count($this->_batch)) {
            $batchQueryCounter++;
            $slice = array_splice($this->_batch, 0, 50);
...
//		return ($isCurrData) ? $arData : false;
		return [
            'client_endpoint' => "https://4hvostru.bitrix24.ru/rest/1/1qh2zsubqmfe22e4/",
            'is_web_hook'     => 'Y'
        ];
	}
