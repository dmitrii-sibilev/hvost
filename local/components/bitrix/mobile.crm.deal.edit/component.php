...
//					$arFields['TAX_VALUE'] = isset($result['TAX_VALUE']) ? $result['TAX_VALUE'] : 0.0;
                    $sum = 0;
                    foreach ($arProd as $arProduct) {
                        $sum += $arProduct["PRICE"];
                    }
                    $arFields['OPPORTUNITY'] = $sum;
				}
			}
...
foreach ($arResult['~STAGE_LIST'] as $statusID => $statusTitle)
{
	...
	if ($permissionType > BX_CRM_PERM_NONE || $statusID == \Starlabs\Project\Grooming\Deal::getWonStatusId() || $statusID == \Starlabs\Project\Grooming\Deal::getNotComeStatusId())
	{
