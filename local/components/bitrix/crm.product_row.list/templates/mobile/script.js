...
BX.Mobile.Crm.ProductEditor = {
	...
	generateProductHtml: function(product)
	{
		...
		if (this.isEditMode) // quantity
		{
			...
			childrenNodesEdit.push(
				...
				BX.create("input", {
					attrs: {
						"data-role": "productPriceInput",
						className: "crm-mobile-product-view-input-price",
						value: ttl,
					...
					events: {
						keyup : function () {
							this.changeProductPrice(product.DATA_ROLE);
						}.bind(this)
					}
				}),
			);
	...
	changeProductPrice : function(dataRole)
	{
		var productNode = this.productsContainerNode.querySelector("[data-role='"+ dataRole +"']");
		var priceInput = productNode.querySelector("[data-role='productPriceInput']");

		if (typeof priceInput === "object")
		{
			var curPrice = priceInput.value;

			var priceNode = productNode.querySelector("[data-role='productPrice']");
			if (typeof priceNode === "object")
				priceNode.innerHTML = this._currencyFormat.replace(/(^|[^&])#/g, curPrice)

			for (i=0; i<this.products.length; i++)
			{
				if (this.products[i].DATA_ROLE == dataRole)
				{
					this.products[i].PRICE = typeof(curPrice) != 'undefined' ? parseFloat(curPrice).toFixed(2) : '0.00';
					this.products[i].PRICE_ACCOUNT = typeof(curPrice) != 'undefined' ? parseFloat(curPrice).toFixed(2) : '0.00';
					this.products[i].PRICE_BRUTTO = typeof(curPrice) != 'undefined' ? parseFloat(curPrice).toFixed(0) : '0';
					this.products[i].PRICE_EXCLUSIVE = typeof(curPrice) != 'undefined' ? parseFloat(curPrice).toFixed(0) : '0';
					this.products[i].PRICE_NETTO = typeof(curPrice) != 'undefined' ? parseFloat(curPrice).toFixed(0) : '0';
					this.products[i].FORMATTED_PRICE = this._currencyFormat.replace(/(^|[^&])#/g, curPrice);
					...
					if (this.products[i].QUANTITY)
					{
						...
						if (sumWithQuantityNode)
						{
							var ttl = typeof(this.products[i].QUANTITY * curPrice) != 'undefined' ? parseFloat(this.products[i].QUANTITY * curPrice).toFixed(2) : '0.00';
							var sum =  this._currencyFormat.replace(/(^|[^&])#/g, ttl);
