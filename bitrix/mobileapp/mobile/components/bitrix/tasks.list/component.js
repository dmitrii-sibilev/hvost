...
(() => {
	...
	class TaskList
	{
		...
		onReloadSuccess(response, showLoading, taskListOffset)
		{
			...
			const items = [];
			//region tasks list for masters update
			...
			allTasks.forEach((row) => {
				...
				const taskDateStart = new Date(row.startDatePlan);
				const taskDateEnd = new Date(row.endDatePlan);
				let statePrefix = '';
				const arWeekDays = [
					'вс',
					'пн',
				...
				if (
					today.getDate() === taskDateStart.getDate()
					&& today.getMonth() === taskDateStart.getMonth()
					&& today.getFullYear() === taskDateStart.getFullYear()
				){
					statePrefix = 'Сегодня ';
				} else if (
					tomorrow.getDate() === taskDateStart.getDate()
					&& tomorrow.getMonth() === taskDateStart.getMonth()
					&& tomorrow.getFullYear() === taskDateStart.getFullYear()
				) {
					statePrefix = 'Завтра ';
				} else {
					const day = (taskDateStart.getDate() > 9) ? taskDateStart.getDate() : '0' + taskDateStart.getDate();
					const month = (taskDateStart.getMonth() + 1 > 9) ? taskDateStart.getMonth() + 1 : '0' + Number(taskDateStart.getMonth() + 1);
					statePrefix = arWeekDays[taskDateStart.getDay()] + '., ' + day + '.' + month + ' ';
				}
				const hoursStart = (taskDateStart.getHours() > 9) ? taskDateStart.getHours() : '0' + taskDateStart.getHours();
				const minutesStart = (taskDateStart.getMinutes() > 9) ? taskDateStart.getMinutes() : '0' + taskDateStart.getMinutes();
				const hoursEnd = (taskDateEnd.getHours() > 9) ? taskDateEnd.getHours() : '0' + taskDateEnd.getHours();
				const minutesEnd = (taskDateEnd.getMinutes() > 9) ? taskDateEnd.getMinutes() : '0' + taskDateEnd.getMinutes();
				item.state = statePrefix + 'c ' +hoursStart + ':' + minutesStart + ' до ' + hoursEnd + ':' + minutesEnd;
